...
import { View, Container, Text, ListItem, 
    Left, Body, List, Label, Input, Button,Item,Picker,DatePicker,Right,Title,Toast } from 'native-base';
...
class RequestDetails extends Component{
    ...
    componentDidMount = () =>{
      ...
      let data = this.state.extra_expense;
      console.log("--------------checking aarray----");
      console.log(data[1]);
    }
    ...
    _renderButtons = (name) => {
        ...
        //console.log("Request id: "+this.state.userid);
        ...
        if(name == "update" && id == this.state.userid && ((role == "admin" && request_status == "accepted") || (role !== "admin" && request_status == "rejected"))){
          return "flex";
        }
        if((name == 'edit' || name == 'editTouch') && (((role == "admin" || accessto.indexOf("Accept/Reject") != -1) && request_status == "pending") || (role !== "admin" && request_status == "rejected"))){
          return "flex";
        }
        else{
          console.log("else");
    ...
    _handlechange= (name,text) =>{
      ...
      || name == "total_amount" || name == "purchase_rate" || name == "transportation_rate" || name == "finance_cost" 
      || name == "cess_per" || name == "broker_comission" || name == "insurance_cost" || name == "broker_select" || name == "transportation_select" || name == "currency_subtype"){
        errors = this.calculate_amount(errors);
  ...
  cal_Purchase = (errors) =>{
    if(errors.purchase_quantity.err === false && errors.purchase_quantity.value != '' && errors.purchase_quantity.value.length !== 0 && errors.purchase_quantity.value.toString().match(/\D+[^0-9.]/gm) == null){
      errors.extra_purchase_quantity.value = errors.purchase_quantity.value;
    }
    return errors;
  }
  ...
  validate_digits = (errors,name,label) =>{
    ...
    validateInput = async (name) =>{
      ...
      errors = this.calculate_amount(errors);
      errors = this.cal_Purchase(errors);
      let error_check = false;
      let changesummary = '';
      let y = 1;
        for (var x in errors){
                ...
                if(errors[x].err !== null && errors[x].err !== undefined){
                  if(x == "exp1" || x == "exp2" || x == "exp3" || x == "exp4"){
                    if(errors[x].previousvalue !== errors[x].value){
                      changesummary +="\n "+y+". "+errors[x].name+" (Extra Expense)"+" changed from "+errors[x].previousvalue+" to "+errors[x].value;
                      y++;
                    }
                  }
                  else if(errors[x].heading == "Storage Upto" || errors[x].heading == "Payment Date" || errors[x].heading == "Arrival Date" || errors[x].heading == "BL Date" || errors[x].heading == "Confirmation Date"){
                    if(format.Date(errors[x].value) != format.Date(errors[x].previousvalue)){
                      changesummary += "\n"+y+'. '+errors[x].heading+" Changed from "+format.Date(errors[x].previousvalue)+" to "+ format.Date(errors[x].value)+"\n";
                      y++;
                    }
                  }
                  else{
                    if(errors[x].value != errors[x].previousvalue){
                      changesummary += "\n"+y+'. '+errors[x].heading+" Changed from "+errors[x].previousvalue+" to "+ errors[x].value+"\n";
                      y++;
                    }
                  }
    ...
    _singleupdate = async(request_type) =>{
        ...
        if(request_type == 'accepted'){
            ...
            if(choice){
                ...
                this.setState({request_status:'accepted'});
            }
        }
        ...
        else if(request_type == 'added to inventory'){
            choice = await AlertAsync(
              'Accept Request',
              'Are you sure you want to accept this request?',
                [
            ...
            if(choice){
                await this.setState({request_status:'added to inventory'})
            }
    ...
    _update = async(name) =>{
        ...
        if(name == 'update' && this.props.user.details.role !== "admin"){
            await this.setState({request_status:'pending'});
        }
        else if(name == "update" && this.props.user.details.role == "admin"){
  ...
  <Body>
    <Text>{this.state.changesummary.value}</Text>
            ...
            <View style={styles.ButtonContainer}>
                <Button onPress={ () => this._singleupdate('added to inventory') } success disabled={this.state.editable} style={{display:this._renderButtons('accept'),marginTop:5,marginBottom:5}}>
                    <Text>Accept</Text>
                </Button>
                ...
                <Button disabled={this.state.editable} onPress={() => this._singleupdate('added to inventory')} info style={{display:this._renderButtons('addinventory'),marginTop:5,marginBottom:5}}>
