...
class AcceptedRequest extends Component {
    ...
    refetch = async() => {        
        var resp = await request.findall({request_status:'accepted'});
    ...
    _onSingleUpdate = async (params) => {
        console.log("In Single Update Of Accept")
        var choice = false;
        ...
        if(choice){
            ...
            if(resp !== false){
                if(resp.status !== 'error'){
                    ...
                    Toast.show({text: "Bulk was successfully added!",buttonText: "Okay",type: "success"});
                }
                else{
                    if(this._isMounted){
                        this.refetch();
