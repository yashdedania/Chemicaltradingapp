...
class SalesRequestDetails extends Component{
    constructor(props){
        super(props);
        ...
        this.state = {
          ...
          selling_quantity:{value:params.selling_quantity == null ? '': params.selling_quantity.toString(),previousvalue:params == null ? '': params.selling_quantity.toString(),heading:"Selling Quantity",changeable:true,err:false,helperText:'',label:'Q'},
          ...
          selling_type:{value:params.selling_type.type == null ? '': params.selling_type.type,previousvalue:params == null ? '': params.selling_type.type,heading:"Selling Type",changeable:false},
          selling_subtype:{value:params.selling_type.subtype == null ? '': params.selling_type.subtype,previousvalue:params.selling_type.subtype == null ? '': params.selling_type.subtype,heading:"selling SubType",changeable:false},
          ...
          delivery_date:{value:params.delivery_date == null ? '': params.delivery_date,previousvalue:params == null ? '': params.delivery_date,heading:"Delivery Date",changeable:true,err:false,helperText:''},
          ...
          supplier:{value:params.supplier == null ? '': params.supplier,previousvalue:params == null ? '': params.supplier,heading:"Customer",changeable:false},
          ...
          selling_rate:{value:params.selling_rate.usd == null ? '': params.selling_rate.usd,previousvalue:params.selling_rate.usd == null ? '': params.selling_rate.usd,heading:"Selling Rate",changeable:true,err:false,helperText:'',label:'SR'},
          selling_rate_in_inr:{value:params.selling_rate.inr == null ? '': params.selling_rate.inr,previousvalue:params.selling_rate.inr == null ? '': params.selling_rate.inr,heading:"Selling Rate in Inr",changeable:false,label:'SRI'},
          ...
          linkid:params.sales_id == null ? null : params.sales_id.toString(),
          createdAt:{value:params.createdAt == null ? '' : params.createdAt,previousvalue:params == null ? '' : params.createdAt},
          loading:false,
          old_state:params.request == null ? false : params,
          userid:params.user == null ? '' :params.user.id,
          old:params.request == null ? false: params.request,
          extraedit:params.extraedit == null ? null :params.extraedit,
          page:params.page === null ? null : params.page,
          ready_flag:false,
          export_flag:params.selling_type.subtype == "export" ? true : false,
    ...
    _renderButtons = (name) => {
        ...
        if(name =='subtractinventory'){
            return 'none';
        }
        if(name == 'inprocess' && ( id == this.state.userid && (role != "admin" && accessto.indexOf("Accept/Reject") == -1))  && request_status == 'pending'){
          console.log("Fourth if");  
          return 'flex';
        }
        if(name == "delete" && (role == "admin" || accessto.indexOf("Accept/Reject") != -1) && (request_status == "accepted" || request_status == "pending")){
          console.log("fifth if");
          return 'flex';
        }
        if(name == "accept" && (role == "admin" || accessto.indexOf("Accept/Reject") != -1) && request_status == "pending"){
          return "flex";
        }
        if(name == "reject" && (role == "admin" || accessto.indexOf("Accept/Reject") != -1) && request_status == "pending"){
          return "flex";
        }
        if(name == "update"  && ((role == "admin" && request_status == "subtracted from inventory" || request_status == "verified") || (role !== "admin" && id == this.state.userid && (request_status == "rejected"|| request_status == "subtracted from inventory" || request_status == "verified")))){
          return "flex";
        }
        if((name == 'edit' || name == 'editTouch') && (((role == "admin" || accessto.indexOf("Accept/Reject") != -1) && (request_status == "pending" || request_status == "subtracted from inventory")) || (role !== "admin" && id == this.state.userid && (request_status == "rejected"|| request_status == "subtracted from inventory") ))){
          return "flex";
        }
        else{
            return 'none';
    ...
    _handlecheckchange = (name) =>{
      ...
      if(errors.ready_flag){
        ...
        errors.request_status = "subtracted from inventory";
      }
    ...
    _handlechange= (name,text) =>{
      ...
      || name == "currency_rate" || name == "clearance_cost"  || name == "custom_duty_cost" || name == "cess" || name == "finance_cost"|| 
      name == "estimated_month" || name == "payment_terms_from"|| name == "cess_per" || name == "insurance_cost"){
      ...
      || name == "total_amount" || name == "selling_rate" || name == "transportation_rate" || name == "finance_cost" || name == "cess_per" 
      || name == "insurance_cost" || name == "broker_comission" || name == "broker_select" || name == "transportation_select" || name == "currency_subtype"){
  ...
  validate_digits = (errors,name,label) =>{
    ...
    validateInput = async (name) =>{
      ...
      var i = 1;
      for (var x in errors){
        if(errors[x] !== undefined && errors[x] !== null){
          if(errors[x].changeable !== undefined && errors[x].changeable !== null && errors[x].changeable == true){
            console.log("-----X = "+x);  
              if(errors[x].err !== undefined && errors[x].err !== null){
                ...
                else if(errors[x].heading == "Storage Upto" || errors[x].heading == "Payment Date" || errors[x].heading == "Delivery Date" || errors[x].heading == "BL Date" || errors[x].heading == "Confirmation Date"){
        ...
        if(errors.old !== null && errors.old !== undefined && errors.old !== false){
          ...
          if(parseFloat(errors.old.extra_purchase_quantity) < parseFloat(errors.selling_quantity.value)){
            ...
            choice = await AlertAsync(
              'Warning',
              'Purchase Quantity of Link Purchased Order is less than Selling Quantity Please fill Purchase pending form if you want to buy more quantity',
              [
                {text:'Okay',onPress:() => true},
              ],
              {cancelable:false}
    ...
    _singleupdate = async(request_type) =>{
        ...
        if(request_type == 'accepted'){
            ...
            if(choice){
                ...
                await this.setState({request_status:'accepted'});
            }
        }
        else if(request_type == 'deleted'){
            choice = await AlertAsync(
                'Delete Request',
            ...
            if(choice){
                await this.setState({request_status:'deleted'})
            }
        }
        else if(request_type == 'subtracted from inventory'){
          console.log("----subtract update----");
            choice = await AlertAsync(
                'Add to Inventory',
                'Are you sure you want to subtract this bulk from inventory?',
                [
                  {text:'Yes',onPress:() => true},
                  {text:'No',onPress:() => false}
                ],
            ...
            if(choice){
                await this.setState({request_status:'subtracted from inventory'})
                body.rest = this.state.old_state === false ? false : this.state.old_state;
            }

        }
         
          body.request_status = this.state.request_status;
          body.id = this.state.id.value;
        if(choice){
          console.log(body);
    ...
    _update = async(name) =>{
        ...
        if(name == 'rejected'){
            ...
            if(choice){
                await this.setState({request_status:'rejected'})
            } 
        }
        else{
            choice = await AlertAsync(
            ...
            if(choice){
              if(name == 'update' && this.props.user.details.role !== "admin" && this.state.page !== "inventory"){
                await this.setState({request_status:'pending'});
              }
              else if(name == "update" && this.props.user.details.role == "admin" && this.state.page !== "inventory"){
                await this.setState({request_status:'accepted'});
              }
              else if(name == "update" && this.state.page == "inventory" && this.state.ready_flag){
                await this.setState({request_status:'verified'});
              }
              else if(name == "update" && this.state.page == "inventory" && this.state.ready_flag !== true){
                await this.setState({request_status:'subtracted from inventory'});
              }
            }
        }
        if(choice){
          ...
          let resp = await sales.update(this.state);
          if(resp !== false){
              if(resp.status !== 'error'){
                  console.log("Chemical successfully Updated");
                  Toast.show({text: "Order Details successfully updated",buttonText: "Okay",type: "success"});
                  if(this.state.page == "inventory"){
                    this.props.navigation.navigate("RecentSales");
                  }
                  else{
                    this.props.navigation.navigate("RecentSales");
                  }
              ...
              Toast.show({text:"Error connecting to server",buttonText: "Okay",type: "warning"});
              console.log("False data recieved in Request Details");
              if(this.state.page == "inventory"){
                this.props.navigation.navigate("RecentSales");
              }
        ...
        return(
            ...
            <View style={hstyles.headerContainer}>
              ...
              <Body style={hstyles.body}>
                <Title style={hstyles.headtitle}>Sales Request Details</Title>
              </Body>
...
<ListItem style={fstyles.formContainer}>
  <Left style={fstyles.formLeft}><Label style={styles.inputlabel}>Sales Type:</Label></Left>
  <Body style={fstyles.formBody}> 
    <Text>{this.state.linkid == null ? 'Not Linked(Purchase Pending)' : 'Linked to CC No.'+this.state.linkid}</Text>
  </Body>
</ListItem>
  ...
  <Body style={fstyles.formBody}>
    <Item style={styles.disable}>
      ...
      > 
        ...
        <Picker.Item label="Indent Sales" value="Indent Sales" />
      </Picker> 
    </Item>
    <Text style={gstyles.helpertext}>{this.state.selling_type.helperText}</Text>
</Body>
</ListItem>
...
<ListItem style={fstyles.formContainer}>
    ...
    <Body style={fstyles.formBody}>
      ...
      <Text style={gstyles.helpertext}>{this.state.selling_subtype.helperText}</Text>
    </Body>
</ListItem>
...
<ListItem style={fstyles.formContainer}>
  <Left style={fstyles.formLeft}><Label style={styles.inputlabel}>Selling Quantity(MT): <Text style={styles.formulaLabel}>{this.state.selling_quantity.label}</Text></Label></Left>
  <Body style={fstyles.formBody}>
    <Item style={this.state.extraedit === null ? (this.state.editable ? styles.enable:styles.disable):styles.disable}>
      <Input disabled={this.state.extraedit === null ? !this.state.editable : true} value={this.state.selling_quantity.value} keyboardType="numeric" onChangeText={(text) => this._handlechange('selling_quantity',text)}   />
    </Item>
    <Text style={gstyles.helpertext}>{this.state.selling_quantity.helperText}</Text>
  </Body>
</ListItem>
...
<ListItem style={fstyles.formContainer}>
  ...
  <Body style={fstyles.formBody}>
    <Item style={(this.state.old === false || this.state.old === null ) ? (this.state.editable ? styles.enable:styles.disable) : styles.disable} >
    <Picker
          mode="dropdown"
          selectedValue={this.state.delivery_place.value}
          enabled={(this.state.old === false || this.state.old === null ) ? this.state.editable : false}
  ...
  <Body style={fstyles.formBody}>  
    ...
    <Text style={gstyles.helpertext}>{this.state.bl_date.helperText}</Text>
              ...
              <ListItem style={fstyles.formContainer}>
                  ...
                  <Body style={fstyles.formBody}>  
                    <Item style={this.state.editable ? styles.enable:styles.disable} >
                      <Input keyboardType="numeric" disabled={!this.state.editable} value={this.state.payment_terms.value} onChangeText={(text) => this._handlechange('payment_terms',text)} />
                    </Item>
...
<ListItem itemDivider style={fstyles.divider}>
  ...
  <ListItem style={fstyles.formContainer}>
    ...
    <Body style={fstyles.formBody}>
      <Item style={this.state.editable ? styles.enable:styles.disable}>
        <Picker enabled={this.state.export_flag === false? false : this.state.editable} mode="dropdown" selectedValue={this.state.currency_type.value} onValueChange={(text) => this._handlechange('currency_type',text)}>
          <Picker.Item label= "Select option" value="" />
...
<ListItem style={fstyles.formContainer}>
  ...
  <Body style={fstyles.formBody}>
    <Item style={this.state.editable ? styles.enable:styles.disable} >
      <Input disabled={!this.state.editable} value={this.state.selling_rate.value} onChangeText={(text) => this._handlechange('selling_rate',text)} keyboardType="numeric"/>
    </Item>
    <Text style={gstyles.helpertext}> {this.state.selling_rate.helperText} </Text>
  </Body>
</ListItem>
...
<ListItem style={fstyles.formContainer}>
  ...
  <Body style={fstyles.formBody}>  
    ...
    <Text style={gstyles.helpertext}>{this.state.selling_rate_in_inr.helperText}</Text>
  </Body>
</ListItem>
}
...
{!!this.state.export_flag ? (
  <ListItem style={fstyles.formContainer}>
    ...
    <Body style={fstyles.formBody}>
      <Item style={this.state.editable ? styles.enable:styles.disable}>
        <Input disabled={!this.state.editable} value={this.state.cess_per.value} maxLength={3} onChangeText={(text) => this._handlechange('cess_per',text)} keyboardType="numeric"/>
      </Item>
      <Text style={gstyles.helpertext}>{this.state.cess_per.helperText}</Text>
    </Body>
  </ListItem>
):null}
  ...
  <Body>
    <Text>{this.state.changesummary.value}</Text>
            ...
            <View style={styles.ButtonContainer}>
                <Button onPress={ () => this._singleupdate('subtracted from inventory') } success disabled={this.state.editable} style={{display:this._renderButtons('accept'),marginTop:5,marginBottom:5}}>
                    <Text>Accept</Text>
                </Button>
                <Button onPress={() => this.validateInput('rejected')} danger style={{display:this._renderButtons('reject'),marginTop:5,marginBottom:5}} >
                    <Text>Reject</Text>
                </Button>

                <Button onPress={() => this._singleupdate('deleted')} warning style={{display:this._renderButtons('delete'),marginTop:5,marginBottom:5}}>
                    <Text>Delete</Text>
                </Button>

                <Button onPress={() => this.validateInput('update')} primary style={{display:this._renderButtons('update'),marginTop:5,marginBottom:5}}>
                    <Text>Update details </Text>
                </Button>

                <Button disabled={this.state.editable} onPress={() => this._singleupdate('subtracted from inventory')}  info style={{display:this._renderButtons('subtractinventory'),marginTop:5,marginBottom:5}}>
                    <Text>Subtract from Inventory</Text>
                </Button>
                <View style={[styles.inProcess,{display:this._renderButtons('inprocess')}]}><Text style={styles.inProcessText}>In Process.....</Text></View>   
            </View>
        </Container>
        );
    }
...
export default (connect(mapStateToProps)(SalesRequestDetails));