...
class AcceptedRequest extends Component {
    ...
    static navigationOptions = {
        ...
        tabBarIcon:({tintColor}) =>(
            <MaterialCommunityIcons name='check-circle' color={tintColor} size={20} />
        )
    };
    constructor(props){
        super(props);
    ...
    refetch = async() => {
        var resp = await sales.findall({request_status:'accepted'});
    ...
    _onSingleUpdate = async (params) => {
        ...
        if(choice){
            ...
            body.id = params.id;
            body.rest = params.request == null ? null : params;  
            if(this._isMounted){
                this.setState({loading:true});
            }
            ...
            if(resp !== false){
                if(resp.status !== 'error'){
                    ...
                    Toast.show({text: "Bulk was successfully subtracted!",buttonText: "Okay",type: "success"});
                }
                else{
                    if(this._isMounted){
                        this.refetch();
    ...
    _renderItem = ({item}) =>(
        <MylistView
            ...
            extrabut = "Subtract from Inventory"
        />
    );
    _keyboardDidShow = () =>{
                        ...
                        <Body style={hstyles.body}>
                            {!!this.state.searchbarVisible ? (
                               <Input style={hstyles.inputField} placeholder="Search"  onChangeText={(text) => this._handleSearch(text)} value={this.state.query} /> 
                                ...
                                <Title style={hstyles.headtitle}>Accepted Sales Request</Title>
                            )}
