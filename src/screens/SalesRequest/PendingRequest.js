...
class PendingRequest extends Component {
    static navigationOptions = {
        ...
        tabBarIcon:({tintColor}) =>(
            <AntDesign name='questioncircle' color={tintColor} size={18}/>
        )
    };
    constructor(props){
        super(props);
    ...
    refetch = async() => {        
        var resp = await sales.findall({request_status:'pending'});
        ...
        if(choice){
            ...
            if(resp !== false){
                if(resp.status !== 'error'){
                    ...
                    Toast.show({text: "Request Successfully Accepted!",buttonText: "Okay",type: "success"});
                }
                else{
                    if(this._isMounted){
                        this.refetch();
                        ...
                        <Body style={hstyles.body}>
                            {!!this.state.searchbarVisible ? (
                               <Input style={hstyles.inputField} placeholder="Search"  onChangeText={(text) => this._handleSearch(text)} value={this.state.query} /> 
                                ...
                                <Title style={hstyles.headtitle}>Sales Pending Request</Title>
                            )}
