...
import SalesModal from '../Inventory/SalesModal';
...
class DSalesRequest extends Component {
    ...
    _refetch = async() =>{
        if(this._isMounted){
            ...
            if(resp !== false){
                ...
                console.log("False error in Sales Request");
            }
    ...
    _onPressItem = (params) =>{
        if(this._isMounted){
            this.setState({modalVisible:true,currentData:params}); 
        }
    }
    _onDismiss = () =>{
        if(this._isMounted){
            this.setState({modalVisible:false,currentData:null});
        ...
        return(
            ...
            <ListItem thumbnail style={{flex:1,flexDirection:"row",margin:10}} onPress={() => this._onPressItem(item)}>
                ...
                <Body>
                    ...
                    <Text style={gstyles.textRegular}><Text style={gstyles.textBold}>Total Amount :</Text> Rs. {item.total_amount === null ? '0' : item.total_amount}</Text>
                </Body>
            </ListItem>
            </View>
        );
    ...
    render() {
        ...
        return(
            <Container style={this.state.searchBarFocused ? hstyles.blurBackground : null}>
                        ...
                        <Body style={hstyles.body}>
                            {!!this.state.searchbarVisible ? (
                               <Input style={hstyles.inputField} placeholder="Search"  onChangeText={(text) => this._handleSearch(text)} value={this.state.query} /> 
                                ...
                                <Title style={hstyles.headtitle}>Sales Orders</Title>
                            )}
                ...
                <SalesModal visibile={this.state.modalVisible} data={{...this.state.currentData}} dismiss={this._onDismiss} />
...
export default (connect(mapStateToProps)(DSalesRequest));