        ...
        if(this._isMounted){
            ...
            if(resp !== false){
                if(resp.status !== 'error'){
                    ...
                    /*var newList = resp.result;
                    newList = _.groupBy(newList,d => d.chemical_name);
                    //console.log("newlist-------printing ");
                    //console.log(newList);
                    newList = _.reduce(newList,(acc,next,index) => {
                        acc.push({
                            key:index,
                            data:next
                        })
    ...
    _renderItem = ({item}) =>{
        ...
        return(
            ...
            <ListItem thumbnail style={{flex:1,flexDirection:"row",margin:10}} onPress={() => this._onPressItem(item)}>
                ...
                <Right><TouchableOpacity onPress={() => this._onPressList(item)}><Text style={gstyles.salesText}>Sell</Text></TouchableOpacity></Right>
            </ListItem>
            </View>
        );
                        ...
                        <Body style={hstyles.body}>
                            {!!this.state.searchbarVisible ? (
                               <Input style={hstyles.inputField} placeholder="Search"  onChangeText={(text) => this._handleSearch(text)} value={this.state.query} /> 
                                ...
                                <Title style={hstyles.headtitle}>Ready Orders</Title>
                            )}
