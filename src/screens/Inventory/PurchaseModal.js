...
const PurchaseModal = (props) =>{
    ...
    return(
        ...
        <View style={styles.ModalContainer}><Text style={styles.Modaltitle}>Order Summary</Text><TouchableOpacity onPress={() => props.dismiss()}><MaterialCommunityIcons name='close-outline' style={styles.Modalclose}/></TouchableOpacity></View>
            ...
            {(props.data !== null && props.data !== undefined && JSON.stringify(props.data).length != 2)?
            (
                <Content>
                <List>
                ...
                <ListItem>
                    ...
                    <Body><Text style={styles.listitemstyle}>{props.data.purchase_quantity}</Text></Body>
                </ListItem>

                <ListItem>
                    <Left><Text>Quantity in Stock:</Text></Left>
                    <Body><Text style={styles.listitemstyle}>{props.data.extra_purchase_quantity}</Text></Body>
                </ListItem>
                {props.data.request_status !== "deleted" ? (
                    <ListItem>
                        <Left><Text>Pending Orders:</Text></Left>
                        <Body><Text style={styles.listitemstyle}>{props.data.total_pending == null ? '0' : props.data.total_pending}</Text></Body>
                    </ListItem>
                ):(null)}
