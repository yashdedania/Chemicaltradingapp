...
import PurchaseNotModal from './PurchaseNotModal';
...
class PurchaseNotification extends Component {
    ...
    _fetchData = async() =>{
        var resp = await activity.getactivity({page:'Purchase'});
            let extra = {...this.state};
    ...
    _renderItem = ({item}) =>{
        ...
        if(item.request !== null && item.request !== undefined){
            requestid = item.request.user.id;
        }
        if(role == "admin"){
            return(
                <NotificationList 
    ...
    render() {
        return(
            <Container>
                <PurchaseNotModal visibile={this.state.modalVisible} data={{...this.state.currentparams}} dismiss={this._onDismiss} />
                        ...
                        <Body style={hstyles.body}>
                            {!!this.state.searchbarVisible ? (
                               <Input style={hstyles.inputField} placeholder="Search"  onChangeText={(text) => this._handleSearch(text)} value={this.state.query} /> 
                                ...
                                <Title style={hstyles.headtitle}>Purchase Activity</Title>
                            )}
...
export default (connect(mapStateToProps)(PurchaseNotification));
