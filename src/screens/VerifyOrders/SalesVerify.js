...
class SalesVerify extends Component {
    ...
    _socketConnect = () =>{
        this.socket.on('sales_request',(data) => {
            console.log("in pending Some data recieved: "+data);
            if(this._isMounted){
    ...
    refetch = async() => {
        var resp = await sales.findall({request_status:'subtracted from inventory'});
    ...
    _onPressItem = (params) =>{
        this.props.navigation.navigate('SalesVerifyDetail',{...params});
...
export default (connect(mapStateToProps)(SalesVerify));
