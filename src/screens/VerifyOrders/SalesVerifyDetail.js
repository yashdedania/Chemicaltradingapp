...
class SalesVerifyDetail extends Component{
    constructor(props){
        super(props);
        const { params = {} } = props.navigation.state;
        props.navigation.setParams({ title: 'Order Details' });
        ...
        this.state = {
          ...
          selling_quantity:{value:params.selling_quantity == null ? '': params.selling_quantity.toString(),previousvalue:params == null ? '': params.selling_quantity.toString(),heading:"Selling Quantity",changeable:true,err:false,helperText:''},
          ...
          delivery_date:{value:params.delivery_date == null ? '': params.delivery_date,value:params == null ? '': params.delivery_date,heading:"Delivery Date",changeable:true,err:false,helperText:''},
          ...
          selling_rate:{value:params.selling_rate.usd == null ? '': params.selling_rate.usd,previousvalue:params.selling_rate.usd == null ? '': params.selling_rate.usd,heading:"Selling Rate",changeable:false,err:false,helperText:''},
          selling_rate_in_inr:{value:params.selling_rate.inr == null ? '': params.selling_rate.inr,previousvalue:params.selling_rate.inr == null ? '': params.selling_rate.inr,heading:"Selling Rate in Inr",changeable:false},
          ...
          finance_cost:{value:params.finance_cost == null ? '' : params.finance_cost,previousvalue:params.finance_cost == null ? '' : params.finance_cost,changeable:false,heading:"Finance Cost",err:false,helperText:''},
          ...
          bl_date:{value:params.bl_date== null ? '': params.payment.date,previousvalue:params.bl_date== null ? '': params.payment.date,heading:"BL Date",changeable:false,err:false,helperText:''},
          ...
          changesummary:{value:''? '':params.changesummary,changeable:false},
    ...
    _renderButtons = (name) => {
        ...
        if((name == 'update' || name == 'edit' || name == 'editTouch')  && role == 'seller' && request_status == 'rejected'){
            return 'flex';
        }
        if(name =='subtractinventory' && role == 'seller' && request_status == 'accepted'){
            return 'flex';
        }
        if(name == 'inprocess' && ((role == 'admin' && (request_status == 'rejected' || request_status == 'accepted') ) || (role=='seller' && request_status == 'pending') ) ){
            return 'flex';
        }
        else{
            return 'none';
    ...
    _handlechange= (name,text) =>{
      ...
      if(name == "payment_date" || name == "storage_date" || name == "delivery_date" || name == "bl_date"){
      ...
      || name == "total_amount" || name == "selling_rate" || name == "transportation_rate"){
  ...
  calculate_amount = (errors) =>{
      ...
      let list  = ["selling_quantity","currency_rate","clearance_cost","custom_duty_cost","cess","finance_cost"];
      for (x in list){
      ...
      if(error_check && errors.currency_type.value == ""){
        ...
        if(errors.export_flag){
            total_amount = parseFloat((selling_rate * quantity * currency_rate) + (clearance_cost) + (finance_cost)  + custom_duty_cost + cess);
        }
        else{
          total_amount = parseFloat((selling_rate * quantity));
  ...
  validate_digits = (errors,name,label) =>{
    ...
    validateInput = async (name) =>{
      ...
      var inputlist = ["supplier","vessel_name","payment_date","payment_terms","storage_days","storage_date","extra_storage","delivery_date"];
            ...
            if(errors[x].changeable !== null && errors[x].changeable !== undefined && errors[x].changeable == true){
                if(!errors[x].err){
                  if(errors[x].value != errors[x].previousvalue){
                    if(errors[x].heading == "Storage Upto" || errors[x].heading == "Payment Date" || errors[x].heading == "Delivery Date" || errors[x].heading == "BL Date"){
...
<ListItem style={fstyles.formContainer}>
  ...
  <Body style={fstyles.formBody}>  
    <Item style={styles.disable} >
      <Input disabled={true} value={this.state.linkid == null ? 'Not Linked(Purchase Pending)' : 'Linked to CC No.'+this.state.linkid}/>
    </Item>
  </Body>
</ListItem>
...
<ListItem style={fstyles.formContainer}>
  <Left style={fstyles.formLeft}><Label style={styles.inputlabel}>Selling Quantity :</Label></Left>
  <Body style={fstyles.formBody}>
    <Item style={this.state.editable ? styles.enable:styles.disable}>
      <Input disabled={!this.state.editable} value={this.state.selling_quantity.value} keyboardType="numeric" onChangeText={(text) => this._handlechange('selling_quantity',text)}   />
    </Item>
...
<ListItem style={fstyles.formContainer}>
  ...
  <Body style={fstyles.formBody}>
    <Item style={styles.disable} >
      <Input disabled={true} keyboardType="numeric" value={this.state.packing_weight.value} onChangeText={(text) => this._handlechange('packing_weight',text)}/>
    </Item>
...
{(this.state.estimated_time.value != "" && this.state.estimated_time.value != "ready") ? (
  <ListItem style={fstyles.formContainer}>
    ...
    <Body style={fstyles.formBody}>
      <Item regular style={styles.disable} >
            <Picker
              mode="dropdown"
...
<ListItem style={fstyles.formContainer}>
  <Left style={fstyles.formLeft}><Label style={styles.inputlabel}>Delivery Date :</Label></Left>
  <Body style={fstyles.formBody}>  
    <Item style={this.state.editable ? styles.enable:styles.disable} >
...
<ListItem style={fstyles.formContainer}>
  <Left style={fstyles.formLeft}><Label style={styles.inputlabel}>Extra Storage :</Label></Left>
  <Body style={fstyles.formBody}>  
    <Item style={styles.disable} >
...
{this.state.transportation_select.value === "accord" ? (
  <ListItem style={fstyles.formContainer}>
    <Left style={fstyles.formLeft}><Label style={styles.inputlabel}>Transportation Rate:</Label></Left>
    <Body style={fstyles.formBody}>    
      <Item style={styles.disable} >
...
<ListItem style={fstyles.formContainer}>
  <Left style={fstyles.formLeft}><Label style={styles.inputlabel}>Selling Rate:</Label></Left>
  <Body style={fstyles.formBody}>
    <Item style={styles.disable} >
      <Input disabled={true} value={this.state.selling_rate.value} onChangeText={(text) => this._handlechange('selling_rate',text)} keyboardType="numeric"/>
...
{(this.state.currency_type.value === "usd" || this.state.currency_type.value == "aed") &&
<ListItem style={fstyles.formContainer}>
  <Left style={fstyles.formLeft}><Label style={styles.inputlabel}>Selling Rate (in Inr):</Label></Left>
  <Body style={fstyles.formBody}>  
    <Item style={styles.disable} >
...
export default (connect(mapStateToProps)(SalesVerifyDetail));